...
export interface DocsAccountMenuUser {
  name: string | null;
  email: string;
  image: string | null;
}
...
/**
 * The account avatar + dropdown, shared by `DocsHeader` (desktop) and
 * `DocsMobileHeader` (mobile) — extracted so both headers show the exact
 ...
 * kanban.local`), the first plugin to make this same `shell: minimal`
 * migration.
 *
 ...
 */
export function DocsAccountMenu({
  user,
  avatarSize = 'md',
}: {
